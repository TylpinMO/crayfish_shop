# 🐟 Fish Shop - E-commerce Template

🚀 **Portfolio Project** | Полнофункциональный интернет-магазин с админ-панелью

> **Современный e-commerce шаблон** - демонстрация full-stack навыков: от фронтенда до безопасной админки с базой данных.

## ⚡ Быстрый старт

**Хотите развернуть за 10 минут?** → Читайте [`QUICK_START.md`](QUICK_START.md)

**Подробная настройка** → [`docs/SETUP_SUPABASE.md`](docs/SETUP_SUPABASE.md) и [`docs/SETUP_NETLIFY.md`](docs/SETUP_NETLIFY.md)

## � Необходимые токены

Для полной функциональности проекта вам понадобятся:

### 1. Supabase (База данных + Админка)

- **Где получить**: https://supabase.com → создать проект
- **Что нужно**: Project URL и Service Role Key
- **Зачем**: Хранение товаров, заказов, авторизация админки

### 2. DaData (Автокомплит адресов) - опционально

- **Где получить**: https://dadata.ru/api/ → регистрация
- **Что нужно**: API Token
- **Зачем**: Автодополнение адресов при оформлении заказа

### 3. JWT Secret (Безопасность админки)

- **Где получить**: Сгенерировать на https://generate-secret.now.sh/32
- **Что нужно**: Случайная строка
- **Зачем**: Подписывание токенов авторизации

**💡 Все токены настраиваются через переменные окружения Netlify - никаких секретов в коде!**
